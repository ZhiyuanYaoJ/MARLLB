...
def calcul_fair(values, type):
    global fair_fn
    return fair_fn[type](values)
...
class LoadBalanceEnv(gym.Env):
    ...
    def __init__(self, interval, logger=None, verbose=False, gt=False, reward_field='flow_duration_avg_decay', momentum=0.1, discrete=False, reward_type='var'):
        '''
        @brief:
        ...
        self.n_flow_on_idx = sm.FEATURE_AS_ALL.index('n_flow_on')
        self.fct_avg_idx = sm.FEATURE_AS_ALL.index('fct_avg')
        self.flow_duration_avg_idx = sm.FEATURE_AS_ALL.index('flow_duration_avg')
    ...
    def _calcul_reward(self, active_as, feature_as, gt=None):
        ...
        for asid in active_as:
            # number of flows * (FCT - flow_duration)
            # reward_field_value = feature_as[asid, self.n_flow_on_idx]*(
            #     feature_as[asid, self.fct_avg_idx]-feature_as[asid, self.flow_duration_avg_idx])
            
            # FCT - flow_duration
            # reward_field_value = feature_as[asid, self.fct_avg_idx]-feature_as[asid, self.flow_duration_avg_idx]

            reward_field_value = feature_as[asid, self.reward_field_idx]
            ...
            # self.logger.info(
            #     "env: asid [{}] reward_field_value = n_flow_on * (fct - flow duration) | {:.6f} = {}*({:.6f}-{:.6f})".format(
            #         asid,
            #         reward_field_value,
            #         feature_as[asid, self.n_flow_on_idx],
            #         feature_as[asid, self.fct_avg_idx],
            #         feature_as[asid, self.flow_duration_avg_idx]
            #         ))
        ...
        >> option 3: fairness
        '''
        reward = calcul_fair(reward_field_values, self.reward_type)
